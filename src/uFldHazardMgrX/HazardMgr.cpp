...
bool HazardMgr::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    if(key == "UHZ_CONFIG_ACK") 
      ...
      {
	string hazardset_report;      	
	hazardset_report = handleMailReportRequest();
	hazardset_report = stringtrasform(hazardset_report);
	buildNodeMessage(hazardset_report);
      }
    else if(key == "UHZ_MISSION_PARAMS") 
      handleMailMissionParams(sval);
    else if(key == "HAZARDSET_REPORT_JAKE") 
      {
	m_hazardreport_jake = sval;
	m_hazardreport_jake=reversetransform(m_hazardreport_jake);//
	m_hazardreport_kasper = handleMailReportRequest();
	m_hazardreport_jake = removename(m_hazardreport_jake);
	string msg = m_hazardreport_kasper + m_hazardreport_jake;
	Notify("FINAL_REPORT",msg);
      }
...
void HazardMgr::registerVariables()
{
  ...
  Register("HAZARDSET_REPORT_JAKE", 0);
}
...
string HazardMgr::handleMailReportRequest()
{
  m_summary_reports++;
...
//------------------------------------------------------------
// Procedure: buildNodeMessage()

void HazardMgr::buildNodeMessage(string a) 
{
	if(m_report_name == "jake")
	{
		string msg = "src_node=jake,dest_node=kasper,var_name=HAZARDSET_REPORT_JAKE,string_val=" + a;
		//Notify("CHECK", msg); //debug
		Notify("NODE_MESSAGE_LOCAL",msg);
	}
}

//------------------------------------------------------------
// Procedure: stringtrasform()

string HazardMgr::stringtrasform(string b) 
{
	vector<string> svector = parseString(b, ',');
	unsigned int i, vsize = svector.size();
	string msg; 
	for(i=0; i<vsize; i++)
	{
	...
	//Notify("MSG",msg); //debug
	return(msg);
}
//------------------------------------------------------------
// Procedure: reversetransform()

string HazardMgr::reversetransform(string c) 
{
	vector<string> svector = parseString(c, '&');
	unsigned int i, vsize = svector.size();
	string msg; 
	for(i=0; i<vsize; i++)
	{
...
string HazardMgr::removename(string d)
{
	vector<string> svector = parseString(d, '#');
	unsigned int i, vsize = svector.size();
	string msg; 
	for(i=2; i<vsize; i++)
	{
	...
	Notify("0",svector[0]); //debug
	Notify("1",svector[1]); //debug
	Notify("MSG",msg); //debug
	return(msg);
}

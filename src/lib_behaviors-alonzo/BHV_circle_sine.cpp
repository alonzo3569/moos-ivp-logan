...
BHV_circle_sine::BHV_circle_sine(IvPDomain domain) :
  IvPBehavior(domain)
{
...
bool BHV_circle_sine::setParam(string param, string val)
{
...
void BHV_circle_sine::onSetParamComplete()
{
}
...
void BHV_circle_sine::onHelmStart()
{
}
...
void BHV_circle_sine::onIdleState()
{
}
...
void BHV_circle_sine::onCompleteState()
{
}
...
void BHV_circle_sine::postConfigStatus()
{
}
...
void BHV_circle_sine::onIdleToRunState()
{
...
void BHV_circle_sine::onRunToIdleState()
{
}
...
IvPFunction* BHV_circle_sine::onRunState()
{
  ...
  {
	  for(i=1;i<m_resolution+1;i++)
 	 {	
  		m_circle_x = m_center_x + m_r*cos((360/m_resolution)*i*PI/180.0);
  		m_circle_y = m_center_y + m_r*sin((360/m_resolution)*i*PI/180.0);	
		//Mark point on pMarineViewer
		XYPoint point(m_circle_x, m_circle_y);
		stringstream s;
		string label;
		s << i;
		s >> label;
  ...
  {
  	m_circle_x = m_center_x + m_r*cos((360/m_resolution)*counter*PI/180.0);
  	m_circle_y = m_center_y + m_r*sin((360/m_resolution)*counter*PI/180.0);
	m_nextpt.set_vx(m_circle_x);
	m_nextpt.set_vy(m_circle_y);
	//counter++
  }
...
IvPFunction *BHV_circle_sine::buildFunctionWithZAIC() 
{
